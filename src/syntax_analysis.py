...
_PROD_LABELS: dict[str, str] = {
    ...
    'p_printf_stmt':        'output_stmt  →  printf ( format, args )',
    'p_input_stmt':         'input_stmt  →  scanf ( format, &var )',
    'p_for_loop':           'for_loop  →  for ( init ; cond ; update ) { ... }',
    'p_for_init':           'for_init  →  type id = expr  |  id = expr  |  id++/++id',
    'p_for_update':         'for_update  →  id++/++id  |  id = expr  |  id op= expr',
    'p_for_body':           'for_body  →  ε  [emit conditional exit]',
    'p_for_end':            'for_end  →  ε  [emit update + back-jump]',
    'p_break_stmt':         'break_stmt  →  break ;',
    'p_continue_stmt':      'continue_stmt  →  continue ;',
    'p_compound_assign':    'compound_assign  →  id op= expr ;',
    'p_increment_stmt':     'increment_stmt  →  id++ ;  |  id-- ;',
    'p_expr_or':            'expr  →  expr || expr',
    'p_expr_and':           'expr  →  expr && expr',
    'p_expr_not':           'expr  →  ! expr',
    'p_expr_cmp':           'expr  →  expr rel_op expr  [inline comparison]',
    'p_base_increment':     'base  →  IDENTIFIER ++  |  IDENTIFIER --',
    'p_base_string':        'base  →  STRING_LITERAL',
    'p_base_uminus':        'base  →  - base  [unary minus]',
    'p_comparison_expr':    'comparison  →  expr  [bare expression as condition]',
